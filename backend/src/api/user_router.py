...
@router.post("/profile/modify", response_model=ApiResponse)
def update_profile(body: UserUpdate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    service = UserService(db)
    updated_user = service.update_user(current_user["user_id"], body)
...
def change_password(body: ChangePassword, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    service = UserService(db)
    service.change_password(current_user["user_id"], body.current_password, body.new_password)
...
async def upload_avatar(
    ...
    filename = f"{current_user['user_id']}_{timestamp}_{file.filename}"
    ...
    updated_user = service.update_avatar(current_user["user_id"], avatar_url)
...
@router.post("/settings/modify", response_model=ApiResponse)
def update_settings(
    settings: dict,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    service = UserService(db)
    updated_settings = service.update_user_settings(current_user["user_id"], settings)
